...
//		}
	...
	@RequestMapping(value = "/utags/{uid}", method = RequestMethod.GET)
	public List getUserTag(@PathVariable("uid") String uid) {
		...
		List<String> tagList = Optional.ofNullable(userTags)
				.map(tags -> tags.stream().map(UserTag::getTag).collect(Collectors.toList()))
				.orElse(null);
	...
	public void updateTag(@PathVariable("uid") String uid, @PathVariable("tag") String tag, @PathVariable("opr") int opr) {
		...
		UserTag byTag = userTagService.findByTag(uid, tag);
		if (byTag == null) {
			userTagService.addTag(uid, tag);
		}

		userTagService.update(uid, tag, opr);
	}
